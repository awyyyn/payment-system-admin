				...
				<main className="drawer md:drawer-open transition-all ">
					...
					<div className="drawer-content flex flex-col">
						...
						<Header
							...
							role={
								userData.role == "collector" ? "Collector" : "Administrator"
							}
						/>
						{/* Page content here */}
					...
					<div className={`drawer-side overflow-hidden transition-all `}>
						...
						<ul className="menu p-4 w-80 bg-yellow-200 h-screen block  overflow-y-auto">
							...
							<h1 className="mx-auto font-bold text-2xl my-2 text-center capitalize">
								{activeName}
							</h1>
							{
								<NavLinks
									handleClick={handleClicke}
									navLinks={
										userData.role == "collector" ? collectorLinks : adminLinks
									}
								/>
							}
...
const NavLinks = ({ navLinks, handleClick }) => {
	return navLinks.map(({ name, path, icon }, i) => (
		<li
			key={i}
			onClick={(e) => handleClick(e)}
			className={`text-xl my-1 cursor-pointer  rounded-lg ${
				location.pathname == path ? "bg-slate-900 bg-opacity-10" : ""
			}`}>
			<NavLink to={`${path}`}>
