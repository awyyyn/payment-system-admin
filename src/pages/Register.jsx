...
const Register = () => {
    ...
    const handleSignIn = async() => {
        ...
        const { data: userDataRes } = await supabase.from('lending_corp').select().eq('email', formData.email).single();
        ...
        const encryptedData = CryptoJS.AES.encrypt(JSON.stringify(userDataRes), 'secretKey').toString();
                ...
                <div className='rounded-lg shadow-xl shadow-orange-400  z-50 w-[85vw] md:w-full  p-5 flex flex-col items-center gap-2'>
                    ...
                    <div className='flex  flex-col w-full gap-y-5'>
                        ...
                        <div className='space-y-2 flex flex-col'>
                            <button className={`btn block ${verifying ? 'bg-slate-100' : 'bg-yellow-300 hover:bg-yellow-400 active:bg-yellow-500'}  mb-2`} onClick={handleSignIn}>
                                {verifying ?
                                    <>
