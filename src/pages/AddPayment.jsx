...
const AddPayment = () => {
	...
	const handleSelect = async (client) => {
		const { data } = await supabase
			.from("loans_table")
			.select(`*`)
			.match({ client_id: client.uuid, is_paid: false })
			.single();
		// console.log(data.id)
					...
					<div className="w-full flex flex-wrap flex-col gap-y-5 h-full md:mt-20 items-center justify-center">
						...
						<div className="form-control w-full max-w-xs">
							...
							<h1
								...
								className={`${
									client.uuid ? "text-black" : "text-gray-400"
								} input input-bordered justify-between flex items-center cursor-pointer hover:focus-warning w-full max-w-xs capitalize`}
								disabled={paying ? true : false}
								onClick={() =>
									client.uuid && !paying && window.my_modal_4.showModal()
								}>
								{payment?.id ? (
									<>
						...
						<button
							disabled={paying}
							className="btn disabled:cursor-progress w-full bg-yellow-300 hover:bg-yellow-200 mt-4 max-w-xs"
					...
					{loading ? (
						...
						paymentsInfo?.map((item, index) => (
							<button
								...
								className={`w-full my-2 px-4 capitalize py-2 rounded-lg gap-y-3  cursor-pointer btn-ghost flex items-center justify-between ${
									item.is_paid &&
									"bg-green-300 bg-opacity-50 hover:bg-green-300 hover:bg-opacity-50"
								}`}>
								<h1>{item.date}</h1>
								<h1>{item.amount}</h1>
							</button>
